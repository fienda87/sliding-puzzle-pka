...
from utils.helpers import generate_random_solvable_puzzle, is_solvable
from lib.cache import SolverCache
...
def main():
    ...
    solver_cache = SolverCache(max_entries=50)
                ...
                if event.button == 1:
                    if game_state == "MENU":
                        ...
                        if difficulty:
                            ...
                            if level_data.get('type') == 'random':
                                random_board = generate_random_solvable_puzzle(level_data['grid_size'])
                                game = PuzzleGame(random_board, level_data['goal'])
                            else:
                        ...
                        if action == 'tile_click' and not game.is_animating:
                            ...
                            if not is_solvable(game.current_board, game.goal_board):
                                print("[SOLVER] Puzzle is unsolvable!")
                                game_screen.show_unsolvable_alert()
                            else:
                                print("[SOLVER] Running BFS solver...")
                                cached_result = solver_cache.get(game.current_board, 'BFS')
                                
                                if cached_result:
                                    print("[SOLVER] Using cached result")
                                    result = cached_result
                                else:
                                    result = solve_bfs(game.current_board, game.goal_board)
                                    if result:
                                        solver_cache.set(game.current_board, 'BFS', result)
                            ...
                            if not is_solvable(game.current_board, game.goal_board):
                                ...
                                cached_result = solver_cache.get(game.current_board, 'DFS')
                                ...
                                if cached_result:
                                    ...
                                    if result:
                                        solver_cache.set(game.current_board, 'DFS', result)
