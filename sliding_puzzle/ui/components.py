...
class Modal:
    def __init__(self, window_width, window_height):
        ...
        self.body_font = pygame.font.SysFont(FONT_NAME, 16)
        self.button = None
        self.is_visible = False
        self.title = ""
        self.messages = []
    
    def show(self, title, messages):
        self.title = title
        ...
        button_y = modal_y + modal_height - button_height - 20
        
        self.button = UIButton(button_x, button_y, button_width, button_height, "OK", FONT_SIZE_BUTTON)
    ...
    def render(self, screen):
        ...
        pygame.draw.rect(screen, (255, 255, 255), (modal_x, modal_y, modal_width, modal_height))
        pygame.draw.rect(screen, (200, 0, 0), (modal_x, modal_y, modal_width, modal_height), 3)
        
        title_surface = self.title_font.render(self.title, True, (200, 0, 0))
        title_rect = title_surface.get_rect(center=(self.window_width // 2, modal_y + 30))
        screen.blit(title_surface, title_rect)
        
        y_offset = modal_y + 70
        for message in self.messages:
            message_surface = self.body_font.render(message, True, COLOR_UI_TEXT)
    ...
    def handle_click(self, mouse_pos):
        if self.is_visible and self.button and self.button.is_clicked(mouse_pos):
            self.hide()
            return True
        return False
    
    def update_hover(self, mouse_pos):
        if self.is_visible and self.button:
