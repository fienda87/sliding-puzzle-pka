import pygame
from .components import GameBoard, UIButton, GameUI, Modal
...
class MenuScreen:
    def __init__(self, window_width, window_height):
        ...
        self.buttons = {
            ...
            'random_3x3': UIButton(center_x, start_y + 3 * (button_height + button_spacing), 
                             button_width, button_height, "RANDOM - 3x3, Variable", FONT_SIZE_BUTTON)
        }
...
class GameScreen:
    def __init__(self, window_width, window_height, grid_size):
        ...
        self.modal = Modal(window_width, window_height)
    ...
    def handle_click(self, mouse_pos, game):
        if self.modal.is_visible:
            if self.modal.handle_click(mouse_pos):
                return (None, None)
            return (None, None)
    ...
    def update_hover(self, mouse_pos):
        if self.modal.is_visible:
            self.modal.update_hover(mouse_pos)
        else:
            for button in self.buttons:
                button.update_hover(mouse_pos)
    ...
    def show_unsolvable_alert(self):
        self.modal.show("UNSOLVABLE PUZZLE", [
            "This puzzle configuration is",
            "mathematically unsolvable!",
            "",
            "Try a different puzzle or reset."
        ])
